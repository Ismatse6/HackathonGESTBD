...
# Función que elimina encabezado y pie de página
...
# Función genérica extracción de texto plano
...
def extraer_resultados_aprendizaje(ruta, model):
...
def extraer_temario_asignatura(ruta_pdf):
    ...
    return "\n".join(temario).strip()
        ...
        ### Hacer estructura de temario (arrays con subarrays)
...
def extraer_criterios_evaluacion(pdf_path):
    extraido = extraer_seccion(pdf_path, titulo="Criterios de evaluación", inicio="Criterios de evaluación", fin="Recursos didácticos")
    agnadir = True
    criterios_evaluacion = ""
    for linea in extraido.splitlines():
        if "Recursos didácticos" in linea:
            agnadir = False
        if agnadir:
            criterios_evaluacion += linea + "\n"
